...
TOC_STRUCTURE = [
    ...
    {
        "title": "Tab 1: Performance vs Targets Indicator by Indicator",
        "id": "tab-1",
        "items": [
    ...
    },
    {
        ...
        "items": [
            ...
            {"id": "slide-25", "title": "TPT Performance"}
        ]
    },
    {
        ...
        "items": [
            ...
            {"id": "slide-27", "title": f"HTS_TST Target vs Achievement {DEFAULT_FY} {DEFAULT_QUARTER} "},
            {"id": "slide-28", "title": f"HTS POS Target vs Achievement {DEFAULT_FY} {DEFAULT_QUARTER} "},
...
def update_time_hierarchy(fy, qtr):
    ...
    try:
        ...
        # Add basic quarters from dataset
        unique_qtrs = [opt["value"] for opt in get_unique_options(fy_data, "Quarter")]
        q_opts += [{"label": q, "value": q} for q in unique_qtrs]
        # Append extra quarterly combinations
        for extra_q in ["SAPR1", "Q1_&_Q2_&_Q3", "SAPR2"]:
            q_opts.append({"label": extra_q, "value": extra_q})
        ...
        if q_val == "All":
            ...
            q_data = fy_data.filter(pl.col("Quarter").is_in(["Q1", "Q2"]))
        elif q_val == "Q1_&_Q2_&_Q3":
            q_data = fy_data.filter(pl.col("Quarter").is_in(["Q1", "Q2", "Q3"]))
        elif q_val == "SAPR2":
            q_data = fy_data.filter(pl.col("Quarter").is_in(["Q3", "Q4"]))
        else:
            q_data = fy_data.filter(pl.col("Quarter") == q_val)
...
def get_status_color(achieved_val, annual_target, indicator_name, period_selected, quarter_selected):
    ...
    qtr_map = {"Q1": 3, "Q2": 6, "Q3": 9, "Q4": 12, "SAPR1": 6, "Q1_&_Q2_&_Q3": 9, "SAPR2": 6}
    months_elapsed = 12 
...
def _update_visuals_impl(selected_visual, county, sub, ward, fac, fy, qtr, prd, gen, coarse, finer, palette_name, trend_time_toggle):
    ...
    if qtr != "All":
        if qtr == "SAPR1":
            dff = dff.filter(pl.col("Quarter").is_in(["Q1", "Q2"]))
        elif qtr == "Q1_&_Q2_&_Q3":
            dff = dff.filter(pl.col("Quarter").is_in(["Q1", "Q2", "Q3"]))
        elif qtr == "SAPR2":
            dff = dff.filter(pl.col("Quarter").is_in(["Q3", "Q4"]))
        else:
            dff = dff.filter(pl.col("Quarter") == qtr)
    ...
    # PowerPoint slides (using slide_renderer.py)
        ...
        tab_content = render_pptx_slide(
            slide_id, dff, dff_targets, colors, max_period,
            get_actual, get_target, extract_hts_sdp_metrics,
            county=county, sub=sub, ward=ward, fac=fac
        )
        return tab_content, t6_style
